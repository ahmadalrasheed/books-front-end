...
class MyFavoriteBooks extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    let bookData = await axios.get(`${process.env.REACT_APP_MONGO_API}/getbook?useremail=${emailuser}`)
  ...
  addBook = async (event) => {
    ...
    let BookData = await axios.post(`${process.env.REACT_APP_MONGO_API}/addbook`, BookInfo)
  ...
  deletBook = async (DataId) => {
    let emailuser = this.props.auth0.user.email
    console.log(DataId);
    let DeletedBook = await axios.delete(`${process.env.REACT_APP_MONGO_API}/deletebook/${DataId}?email=${emailuser}`)
  ...
  updatefrommodel = async (event) => {
    ...
    let updateddata = await axios.put(`${process.env.REACT_APP_MONGO_API}/updatebook`, BookInfo)
    this.setState({
